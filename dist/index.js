...
function platformArgument(description) {
  return new Argument("<platform>", description).choices(["ios", "android"]);
}
function createProgram(deps = defaultCliDependencies, { exitOverride = false } = {}) {
  const program2 = new Command;
  program2.name("expo-go").description("Get Expo Go download URLs and binaries").showHelpAfterError().configureOutput({
  ...
  program2.command("url").description("print the Expo Go download URL for a platform").addArgument(platformArgument("Platform to get the Expo Go download URL for")).argument("[sdkVersion]", 'Expo SDK version, or "latest". Defaults to the current project SDK, or latest.').action(async (platform, sdkVersion) => {
    const { url } = await deps.getExpoGoDownloadUrlAsync(platform, { sdkVersion });
    deps.log(url);
  });
  program2.command("download").description("download Expo Go for a platform").addArgument(platformArgument("Platform to download Expo Go for")).argument("[sdkVersion]", 'Expo SDK version to download, or "latest". Defaults to the current project SDK, or latest.').argument("[outputPath]", 'Output path. Defaults to the current directory. Pass an SDK version (or "latest") to use it.').action(async (platform, sdkVersion, outputPath) => {
...
async function runCliAsync(argv = process.argv, deps = defaultCliDependencies, options) {
  const program2 = createProgram(deps, { exitOverride: options?.exitOverride });
  await program2.parseAsync(argv, { from: options?.from ?? "node" });
}
